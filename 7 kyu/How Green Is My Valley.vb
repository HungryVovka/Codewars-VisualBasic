...
' 
' the two wings have the same length. If the length of the array is odd the wings are around the bottom, if the length is even the bottom is 
...
Public Module Valley
    Public Function MakeValley(ByVal arr As Integer()) As Integer()
        Array.Sort(arr)
        Dim newarr As New List(Of Integer)
        Dim arrl = arr.Length
        For i = arrl - 1 To 0 Step -2
            newarr.Add(arr(i))
        Next
        For i = arrl Mod 2 To arrl - 1 Step 2
            newarr.Add(arr(i))
        Next
